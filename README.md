...
| شکل کلی چارت | `chart_shape_json` — رشتهٔ JSON برای ۲۰ کندل اخیر، با **لحظهٔ ورود به معامله به‌عنوان مبدأ صفر**: هر مقدار OHLC به‌صورت `(مقدار - قیمت لحظهٔ ورود) / ATR` است (صفر=دقیقاً محل فعلی قیمت، مثبت=بالای آن، منفی=پایین آن) و `t` تعداد کندل قبل از لحظهٔ ورود است (۱- = آخرین کندل). برای یادگیری الگوهای بزرگ (مثلث، پرچم، دوقلو و ...) از دیدگاه «همین الان کجا ایستاده‌ام»، بدون نیاز به تعریف دستی هر الگو |
